...
def testKNN(X_test,y_test):
    ...
    for key,val in dict_from_csv.items():
        if key.isnumeric():
            ...
            cheak_key = int((trance_key % 10000) / 1000)*1000
            if cheak_key==8000:
    ...
    for key,val in class_report.items():
            try:
                float(key)
                x.append(key)
                y.append(val['f1-score'])
            except:
                continue

    #x = ['Linux', 'Windows', 'OSX']  
    #x = ['Chrome', 'FireFox', 'IExplorer', 'Safari','Non-browser']
    ...
    for i in x:
        split_i = i.split('.')
        if split_i[0] in dict_from_csv:
            t = ' '.join(dict_from_csv[split_i[0]])
            my_label.append(t)
    #print("my_label: ",my_label)
    '''

    #print("floatTemperatures: ",floatTemperatures)
    #print("y: ",y)
    ...
    plt.figure(num=3, figsize=(10,8), dpi=120, facecolor='w', edgecolor='r')
    #plt.hist(density=True, histtype='bar', color=colors, label=l)
    #plt.bar(y_pos + 0, importance_sorted, width=0.5, color = colors, label=l)
    #plt.xticks(y_pos, features_sorted)features_sorted
    ...
    #out_name= 'accuracy_report_{}_.txt'.format(name)#,=target_names
...
def testSVC_RBF(X_test,y_test):
    ...
    scaler = _joblib.load('TrainScalar.joblib.pkl')
    X_testScaled = scaler.transform(X_test)
    y_pred = clf.predict(X_testScaled)
...
def trainTestM(X_train,X_test,y_train,y_test):
    ...
    # DecisionTreeClassifier(max_depth=5),
    RandomForestClassifier(max_depth=5, n_estimators=10, max_features=69),
    AdaBoostClassifier(),
    GaussianNB(),
